...
def auto_tag_all_untagged() -> int:
    ...
    songs = Song.objects.filter(status="active", needs_tagging=True)
    c = 0
    for s in songs:
        res = search_musicbrainz_api(s.title or Path(s.filepath).stem, 1)
        if res:
            tags = res[0]
            tags["needs_tagging"] = False 
            try:
                apply_manual_tags(s, tags)
                c += 1
            except Exception:
                pass
    return c
...
def get_compilation_candidates() -> List[Dict[str, Any]]:
    import sqlite3
...
def merge_compilation(nd_song_ids: List[str], job: Optional[Any] = None) -> int:
    ...
    try:
        with sqlite3.connect(db_path, timeout=10) as conn:
            ...
            # Use the album name from the first song as the unified album name for all
            unified_album_name = ""
            ...
            for nd_id in nd_song_ids:
                ...
                if abs_path.exists():
                    # Update file tags (Unify album name and set album artist)
                    data = {
                        "title": row['title'],
                        "artist": row['artist'],
                    ...
                    # Update Navidrome DB directly for immediate effect
